...
function dkRender(){
 ...
 const header=dkEl('header','dk-header'),title=dkEl('div');title.append(dkEl('span','dk-eyebrow',SHEETS[dkActive.sheet].name.toUpperCase()+' / '+(example?'CUSTOMIZE EXAMPLE':'GUIDED BUILDER')),dkEl('h2','',example?'Make this example yours':r.title),dkEl('p','dk-help',example?example.description:r.description));if(r?.packId)title.append(dkEl('p','dk-help','Data-only recipe pack: '+r.packId));header.append(title,dkBtn('Close ✕',dkClose));root.append(header);
 ...
 const reset=dkBtn('Start over',()=>dkChange(()=>{dkActive.blocks=example?example.original.split('\n').map(text=>({key:'line',values:{text}})):r.parts.filter(p=>p.required).map(p=>({key:p.key,values:dkPartValues(p)}));}));toolbar.append(reset);if(r?.packId)toolbar.append(dkBtn('Recipe packs',rpOpen));root.append(toolbar);
