...
const DK_SCHEMA_EDITOR={
 bash:{mode:'text/x-sh',indentUnit:2},powershell:{mode:'application/x-powershell',indentUnit:4},python:{mode:'python',indentUnit:4},terraform:{mode:'text/x-sh',indentUnit:2,structured:true},sql:{mode:'text/x-sql',indentUnit:2},dockerfile:{mode:'dockerfile',indentUnit:2},docker:{mode:'dockerfile',indentUnit:2,assistId:'dockerfile'},json:{mode:{name:'javascript',json:true},indentUnit:2,structured:true},yaml:{mode:'yaml',indentUnit:2,structured:true,preserveSyntax:'yaml-cst'},ansible:{mode:'yaml',indentUnit:2,structured:true,preserveSyntax:'yaml-cst'},github:{mode:'yaml',indentUnit:2,structured:true,preserveSyntax:'yaml-cst'},'github-actions':{mode:'yaml',indentUnit:2,structured:true,assistId:'github',preserveSyntax:'yaml-cst'},gitlab:{mode:'yaml',indentUnit:2,structured:true,preserveSyntax:'yaml-cst'},'gitlab-ci':{mode:'yaml',indentUnit:2,structured:true,assistId:'gitlab',preserveSyntax:'yaml-cst'},jenkins:{mode:'groovy',indentUnit:2},cicd:{mode:'yaml',indentUnit:2},command:{mode:null,indentUnit:2}
};
const DK_SCHEMA_PROFILE_SHEETS={dockerfile:'docker','github-actions':'cicd','gitlab-ci':'cicd',jenkins:'cicd',yaml:'linux',json:'linux'};
function dkSchemaEditorDefaults(id){const value=DK_SCHEMA_EDITOR[id]||{mode:null,indentUnit:2};return {assistId:value.assistId||id,mode:value.mode??null,indentUnit:value.indentUnit||2,structured:!!value.structured,preserveSyntax:value.preserveSyntax||null};}
function dkSchemaRegister(definition){
 ...
 schema.editor={...dkSchemaEditorDefaults(schema.id),...(schema.editor||{})};if(schema.editor.preserveSyntax&&!['yaml-cst'].includes(schema.editor.preserveSyntax))throw Error('Unsupported syntax preservation mode: '+schema.editor.preserveSyntax);schema.assist={id:schema.editor.assistId,catalog:[...(definition.assist?.catalog||DK_SCHEMA_ASSIST_CATALOG[schema.editor.assistId]||[])]};
