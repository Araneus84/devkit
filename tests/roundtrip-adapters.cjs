...
vm.runInContext(
 ...
 {const p=DD_PROFILES.dockerfile,n=(t,v={},s={})=>ddNode(p,t,v,s);const nodes=[n('comment'),n('from'),n('workdir'),n('env',{value:'literal $HOME and "quotes"'}),n('copy'),n('expose'),n('user'),...['run','cmd','entrypoint'].map(t=>n(t,{}, {args:[n('arg')]}))];round('dockerfile',nodes);const code='ARG VERSION=3.21\nFROM --platform=linux/amd64 alpine:$VERSION AS build\nENV APP_ENV=production PORT=8080\nENV PATH=$PATH:/app\nCOPY --from=builder --chown=1000 package.json package-lock.json /app/\nRUN echo hello && \\\n    echo world\nEXPOSE 80/tcp 53/udp\nCMD node app.js\nENTRYPOINT ["sh", "-c"]\n';const parsed=sxParse(p,code,[]);check(!types(parsed).includes(SX_RAW),'Common Docker forms became custom: '+types(parsed));check(parsed.find(x=>x.type==='run_shell').values.command.includes('echo world'),'Continuation lost');const again=sxParse(p,p.generate(parsed),[]);check(!types(again).includes(SX_RAW),'Docker second round-trip lost blocks');const before=sxParse(p,'FROM alpine\nRUN ["echo", "hello"]\n',[]);try{sxParse(p,'FROM alpine\nRUN ["echo",\n',before);throw Error('Malformed JSON accepted');}catch(e){check(e.message.includes('JSON argument'),'Wrong invalid JSON error');}const hd=sxParse(p,'FROM alpine\nRUN <<EOF\necho hello\nCOPY not an instruction\nEOF\nCMD echo end\n',[]);check(hd.length===3&&hd[1].type===SX_RAW&&hd[2].type==='cmd_shell','Heredoc was split into fake instructions');}
 {const p=DD_PROFILES.sql,n=(t,v={},s={})=>ddNode(p,t,v,s);const nodes=[n('select',{}, {columns:[n('column'),n('aggregate'),n('sqlExpression')],joins:[n('join')],where:[n('group',{}, {conditions:[n('compare'),n('nullcheck'),n('in')]})],group:[n('column')],having:[n('compare',{leftKind:'COUNT',kind:'number',value:'1'})],order:[n('sort')]}),n('insert',{}, {rows:[n('row',{}, {values:[n('sqlValue'),n('sqlValue',{kind:'boolean',value:'true'})]})]}),n('update',{}, {assignments:[n('assignment')],where:[n('compare')]}),n('delete',{scope:'all rows'}),n('create',{}, {columns:[n('definition')]})];round('sql',nodes);round('sql',[n('select',{table:'filtered'},{cte:[n('cte',{}, {query:p.seed(p)})],columns:[n('column')]})]);}
