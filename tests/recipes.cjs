...
for (const tool of report.tools) {
  assert(report.guides.includes(tool), 'Missing guide ' + tool);
  assert(
    report.recipes.some((r) => r.sheet === tool),
    'Missing builder ' + tool,
  );
}
const text = (id) => report.results.find((r) => r.id === id).text;
assert(text('sql:select').includes('\nFROM users'));
assert(text('ssh:config').includes('HostName server.example.com'));
assert.equal(text('excel:lookup'), '=XLOOKUP(A2,D2:D100,E2:E100,"Not found",0)');
assert(text('cmd:copy').includes(' /L '));
assert(text('juniper:interface-label').includes('commit check'));
assert(!/^commit$/m.test(text('juniper:interface-label')));
...
for (const file of manifest.javascript)
  new vm.Script(fs.readFileSync(path.join(root, file), 'utf8'), { filename: file });
for (const file of manifest.styles)
  assert(!/@import/.test(fs.readFileSync(path.join(root, file), 'utf8')), 'Remote CSS dependency');
console.log(
  'PASS: ' +
    report.tools.length +
    ' tools, ' +
    ...
    ' file/workflow recipes, YAML/JSON parsing, source syntax, no remote CSS dependencies.',
);
