...
const c = vm.createContext({ console, samples });
for (const f of [
...
const result = vm.runInContext(
  'samples.map(source=>({source,generated:pyP.generate(pyParse(source))}))',
  c,
);
if (process.env.DEVKIT_TEST_OUTPUT) {
  fs.mkdirSync(process.env.DEVKIT_TEST_OUTPUT, { recursive: true });
  fs.writeFileSync(
    path.join(process.env.DEVKIT_TEST_OUTPUT, 'roundtrip.json'),
    JSON.stringify(result),
  );
}
for (const item of result) {
  c.code = item.generated;
  vm.runInContext('pyParse(code)', c);
}
console.log(
  'PASS: ' +
    samples.length +
    ' Python round-trip fixtures parsed. Set DEVKIT_TEST_OUTPUT to export them for CPython AST comparison.',
);
