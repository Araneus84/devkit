...
vm.runInContext(
 ...
 w.slots.env.push(g('secret'));job.slots.matrix.push(g('axis'));job.slots.steps[2].values.text='echo "a: b"\necho done';
 let data=jsyaml.load(ddGenerate(gh,tree).text);check(data.env.API_TOKEN==='\u0024{{ secrets.API_TOKEN }}','Secret reference changed');check(data.jobs.test.strategy.matrix.python[1]==='3.13','Matrix lost text types');check(data.jobs.test.steps[2].run.includes('\n'),'Multiline changed');check(Object.hasOwn(data,'on'),'Trigger key missing');
 w.slots.jobs.push(g('job',{name:'deploy',needs:'test'},{steps:[g('run')]}));check(!ddGenerate(gh,tree).errors.length,'Valid needs rejected');job.values.needs='deploy';check(ddGenerate(gh,tree).errors.some(x=>x.includes('cycle')),'Cycle accepted');job.values.needs='missing';check(ddGenerate(gh,tree).errors.some(x=>x.includes('Unknown')),'Unknown needs accepted');job.values.needs='';w.slots.jobs[1].values.name='test';check(ddGenerate(gh,tree).errors.length,'Duplicate jobs accepted');
 const gl=DD_PROFILES['gitlab-ci'],l=(t,v={},s={})=>ddNode(gl,t,v,s),pipeline=gl.seed(gl),stages=pipeline[0].slots.stages;
